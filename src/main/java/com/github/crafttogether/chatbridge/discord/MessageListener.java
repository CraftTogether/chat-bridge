...
public class MessageListener {
    ...
    public void onGuildMessageReceived(@NotNull MessageCreateEvent event) {
        ...
        if (event.getMessage().getAuthor().get().getId().equals(event.getClient().getSelfId())) return;
        ...
        try {
            IrcMessageSender.send(String.format("\u000310[Discord]: <%s> %s", event.getMember().get().getDisplayName(), event.getMessage().getContent()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
